...
class PokemonRepository {
  static const _baseImageUrl = 'https://assets.pokemon.com/assets/cms2/img/pokedex/full/';
  ...
  Future<List<Pokemon>> getPokemons({required int page}) async {
    ...
    for (final pokemonListResult in pokemonListDto.results!) {
      ...
      final pokemonTypes = pokemonDto.types!.map((typeHolder) => typeHolder.type!.name!).toList();
  ...
  String _transformPokemonId(int pokemonId) {
    //return pokemonId < 10 ? '00$pokemonId' : pokemonId < 100 ? '0$pokemonId' : pokemonId.toString();
